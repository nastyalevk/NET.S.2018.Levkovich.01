    ...
    public sealed class Polynomial : ICloneable, IEquatable<Polynomial>
    {
        ...
        public static bool operator !=(Polynomial right, Polynomial left)
            ...
            public override string ToString()
            {
                ...
                {
                    string result;
                    return result = "f(x)=" + this.Coefficients;
                }
                else
                {
                    StringBuilder result = new StringBuilder("f(x)=" + this.Coefficients[0] + "x^" + (this.Degree));
                    for (int i = 1; i < this.Coefficients.Length; i++)
                    {
                        result.Append(" + " + this.Coefficients[i] + "x^" + (this.Degree - i));
                    }
